...
// INCREASED SPACING to prevent 3D projection overlap
...
const assignSlot = (existingEnemies = [], myId = null) => {
    ...
    if (available.length === 0) return { 
        slotIndex: 0, 
        x: GRID_SLOTS[0].col * CELL_WIDTH, 
        y: GRID_SLOTS[0].row * CELL_HEIGHT, 
        distance: 300 
    };
...
export const KernelPanic = {
  ...
  setup: () => {
    ...
    return {
        players: {
          '0': { id: '0', name: 'Firewall', hp: 120, maxHp: 120, speed: 5, class: 'Firewall', desc: 'Packet Filter', isDefending: false },
          '1': { id: '1', name: 'Daemon', hp: 70, maxHp: 80, speed: 12, class: 'Daemon', desc: 'Memory Leak', isDefending: false }
        },
        enemies: {
        ...
        phase: 'combat',
        depth: 1,
        log: ['> SYSTEM BOOT', '> SECTOR 1 INITIALIZED']
    };
  },
  ...
  moves: {
    attack: ({ G }, targetId) => {
      ...
      if (enemy.hp === 0) G.log.push(`> ${enemy.name} TERMINATED`);
